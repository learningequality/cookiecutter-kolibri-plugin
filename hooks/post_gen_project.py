...
if (
    "{{ cookiecutter.frontend_plugin }}" != "Yes"
    and "{{ cookiecutter.content_renderer_plugin }}" != "Yes"
):
...
if "{{ cookiecutter.content_renderer_plugin }}" == "Yes":
    ...
    while context["content_kind"] not in dict(content_kinds.choices):
        print(
            "Choose a content kind that this plugin renders, choose from {}".format(
                ", ".join(dict(content_kinds.choices).keys())
            )
        )
